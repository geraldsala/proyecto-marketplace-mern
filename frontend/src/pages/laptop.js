...
import { LinkContainer } from 'react-router-bootstrap'; // Importante para la navegación
...
const LaptopPage = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeProcs.length) {
      data = data.filter(p => p.nombre.toLowerCase().includes(activeProcs.join(' ').toLowerCase()));
    }
      ...
      <Container fluid className="pt-3">
        ...
        <Row className="align-items-center g-3 px-2">
          <Col xs="12" className="breadcrumbs">
            <small>
              <span className="crumb">Inicio</span> / <span className="crumb">Tecnología</span> / <strong>Portátiles</strong>
            </small>
          </Col>
        ...
        <Row className="mt-3 gx-4">
          ...
          <Col lg="3" className="mb-4">
            <aside className="filters card border-0 shadow-sm p-3">
              ...
              <input type="range" min={minPrice} max={maxPrice} step="5000" value={priceMax}
                     onChange={(e)=>setPriceMax(Number(e.target.value))} className="w-100" />
