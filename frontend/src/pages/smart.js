...
const BRANDS = ['Xiaomi', 'Amazon', 'Nexxt'];
const CATEGORIES = ['Parlantes Inteligentes', 'Cámaras de Seguridad'];

// --- CAMBIO 1: ID correcto de la base de datos para "Smart Home" ---
const SMART_CATEGORY_ID = '68a8041b72eae1ed4d97994e';
...
const SmartPage = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                // Se piden TODOS los productos
                const productsFromAPI = await productService.getProducts();
                
                // --- CAMBIO 2: Lógica de filtrado robusta y estandarizada ---
                const smartProducts = productsFromAPI.filter(product => {
                    ...
                    if (typeof product.categoria === 'object' && product.categoria._id) {
                        return product.categoria._id === SMART_CATEGORY_ID;
                    }
                    
                    // Si la categoría es un string (ID), compárala directamente
                    if (typeof product.categoria === 'string') {
                        return product.categoria === SMART_CATEGORY_ID;
                    }

                    return false;
                });
                
                setAllProducts(smartProducts);
            } catch (err) {
                setError('No se pudieron cargar los productos de Smart Home.');
                console.error(err);
            }
        };
        fetchProducts();
            ...
            <Container fluid className="pt-3">
                ...
                <Row className="align-items-center g-3 px-2">
                    <Col xs="12" className="breadcrumbs">
                        <small><span className="crumb">Inicio</span> / <span className="crumb">Tecnología</span> / <strong>Smart Home</strong></small>
                    </Col>
