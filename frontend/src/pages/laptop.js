                ...
                <Row className="mt-3 gx-4">
                    <Col lg="3" className="mb-4">
                        <aside className="filters card border-0 shadow-sm p-3">
                            ...
                            {BRANDS.map((b) => (<Form.Check key={b} type="checkbox" label={b} checked={!!brandChecks[b]} onChange={() => toggleBrand(b)} />))}
                            <hr />
                            ...
                            <div className="checklist">
                                {PROCESSORS.map((p) => (<Form.Check key={p} type="checkbox" label={p} checked={!!procChecks[p]} onChange={() => toggleProc(p)} />))}
                            </div>
                        </aside>
                    </Col>
                    <Col lg="9">
                        ...
                        <div className={gridMode === 'grid' ? 'grid-products' : 'list-products'}>
                            {filtered.slice(0, perPage).map((product) => (
                                <article key={product._id} className="product-card">
                                    {/* --- SECCIÓN DE LA IMAGEN (Esta era la parte que faltaba) --- */}
                                    <div className="thumb">
