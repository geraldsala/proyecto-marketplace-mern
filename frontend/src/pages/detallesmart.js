...
export default function DetallesSmart() {
  ...
  // Calificación/Reseñas (estado local para UI)
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Normalización para evitar null/undefined en la UI
        const safe = {
          ...
          imagenes:
            Array.isArray(data?.imagenes) && data.imagenes.length
              ? data.imagenes
              : [data?.img ?? 'https://via.placeholder.com/600x600?text=Smart+Home'],
        ...
        // Cargar estado de reportes / inhabilitación
  ...
  const especificacionesLista = useMemo(() => {
    ...
    if (e.conectividad) list.push(`Conectividad: ${e.conectividad}`);
    return list;
  }, [product]);
        ...
        <Col lg={6}>
          <div className="card border-0 shadow-sm p-3 h-100 d-flex">
            ...
            <div className="mt-2">
              ...
              <span className="ms-2 text-muted">{product.rating?.toFixed?.(1) ?? '4.6'} / 5</span>
            </div>
      ...
      <Row className="mt-4 gy-4">
        ...
        <Col lg={4}>
          <div className="card border-0 shadow-sm p-3">
            ...
            {product.especificacionesTecnicas?.compatibilidad && (
              <div><strong>Compatibilidad:</strong> {product.especificacionesTecnicas.compatibilidad}</div>
            )}
